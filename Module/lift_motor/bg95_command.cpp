...
void BG95::MandatoryParamEnqueue()
{
	...
	// Primary Velocity controller(position_mode_velocity)
	...
	// Secondary Velocity controller(velocity_mode_velocity)
...
void BG95::RecommendationParamEnqueue()
{
	...
	const int32_t pos_mode_vel = this->pos_mode_vel_;
	const int32_t vel_mode_vel = this->vel_mode_vel_;
	...
	//parameters for  Primary Velocity controller(position_mode_velocity)
	WriteDataEnqueue(0x3300, 0, pos_mode_vel);      // Velocity - rpm

	WriteDataEnqueue(0x3321, 0, pos_mode_vel);      // Velocity Max. Limit - Positive Direction - rpm
	WriteDataEnqueue(0x3323, 0, pos_mode_vel);      // Velocity Max. Limit - Negative Direction - rpm
	...
	// Secondary Velocity controller(velocity_mode_velocity)
	WriteDataEnqueue(0x3504, 0, 0x500);	  //SVel Source of Desired Value
	WriteDataEnqueue(0x3550, 0, 2426);	  //SVel Feedback

	WriteDataEnqueue(0x3500, 0, vel_mode_vel);	  //SVel Desired Value
...
void BG95::SetVelocityControlEnqueue(bool dir)
{
	...
	AsyncWriteDataEnqueue(0x3003, 0, 5);      // device mode "sub velocity mode"

	AsyncWriteDataEnqueue(0x3500, 0, vel_);   // desired velocity (rpm)
...
void BG95::ReadSchduleCommandEnqueue()
{
	...
	ReadDataEnqueue(0x3500, 0, 0);	    //SVel Desired Value
	...
	ReadDataEnqueue(0x3911, 0, 0);      // Motor - Polarity
...
void BG95::SetDirectionCommand()
{
	...
	if(motor_dir == 0x03) motor_dir = 0x00;
	else motor_dir = 0x03;

	AsyncWriteDataEnqueue(0x3911, 0, motor_dir);      // Motor - Polarity
}



void BG95::HomingStartCommand()
{
	WriteDataEnqueue(0x37b0, 0 ,1);   //Homing - Start Command
